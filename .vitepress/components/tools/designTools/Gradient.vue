...
const lchToRgb = (lch: LCH): RGB => {
  ...
  return [
    ...
    Math.round(Math.max(0, Math.min(255, bb * 255)))
  ];
};
...
const interpolateLCH = (lch1: LCH, lch2: LCH, t: number): LCH => {
  ...
  // 亮度插值：
  const l = l1 + (l2 - l1) * t;//Math.pow(t, 0.85);
...
const interpolateColors = (colors: string[], steps: number): string[] => {
  ...
  for (let i = 0; i < colors.length - 1; i++) {
    ...
    //console.log(`Color ${i+1}: RGB ${rgb1}, LCH ${lch1}`);
    //console.log(`Color ${i+2}: RGB ${rgb2}, LCH ${lch2}`);
